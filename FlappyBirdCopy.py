...
pipe_image = pygame.image.load("pipe6.png")
# bird_image = pygame.transform.scale(bird_image, (BIRD_WIDTH, BIRD_HEIGHT))
...
class Pipe:
    def __init__(self, x):
        ...
        self.bottom_height = SCREEN_HEIGHT - self.top_height - PIPE_GAP-100
    ...
    if game_active:
        ...
        for pipe in pipes:
            if pipe.x <= bird.x + BIRD_WIDTH // 2 <= pipe.x + PIPE_WIDTH:
                if bird.y < pipe.top_height or bird.y > SCREEN_HEIGHT - pipe.bottom_height-50:
                    game_active = False
